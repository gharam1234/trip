...
test.describe('게시글 라우팅 기능', () => {
  ...
  test('최신 게시글 (board-002)을 클릭시 상세페이지로 이동해야 함', async ({ page }) => {
    ...
    // 게시글 데이터가 로드될 때까지 대기
    await page.waitForSelector('[role="row"]:nth-child(2)', { timeout: 500 });
    ...
    await expect(page).toHaveURL('/boards/board-002', { timeout: 500 });
  });
  ...
  test('이전 게시글 (board-001)을 클릭시 해당 상세페이지로 이동해야 함', async ({ page }) => {
    ...
    await page.waitForSelector('[role="row"]', { timeout: 500 });
    await page.waitForSelector('[role="row"]:nth-child(2)', { timeout: 500 });
    ...
    await expect(page).toHaveURL('/boards/board-001', { timeout: 500 });
  });
