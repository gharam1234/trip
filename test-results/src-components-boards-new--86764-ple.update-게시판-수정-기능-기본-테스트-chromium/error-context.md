# Page snapshot

```yaml
- generic [ref=e1]:
  - generic [ref=e2]:
    - text: ← → • • •
    - main [ref=e4]:
      - main [ref=e5]:
        - generic [ref=e6]:
          - heading "게시물 등록" [level=1] [ref=e9]
          - generic [ref=e12]:
            - heading "작성자*" [level=2] [ref=e13]
            - textbox [ref=e16]:
              - /placeholder: 작성자를 입력해 주세요.
              - text: 테스트작성자
          - generic [ref=e18]:
            - heading "비밀번호*" [level=2] [ref=e19]
            - textbox [ref=e22]:
              - /placeholder: 비밀번호를 입력해 주세요.
              - text: "1234"
          - generic [ref=e24]:
            - heading "제목*" [level=2] [ref=e25]
            - textbox [ref=e28]:
              - /placeholder: 제목을 입력해 주세요.
              - text: 테스트 제목
          - generic [ref=e32]:
            - heading "내용*" [level=2] [ref=e33]
            - textbox [ref=e36]:
              - /placeholder: 내용을 입력해 주세요.
              - text: 테스트 내용
          - generic [ref=e39]:
            - heading "주소" [level=2] [ref=e40]
            - generic [ref=e41]:
              - textbox [disabled] [ref=e44]:
                - /placeholder: "01234"
              - button "우편번호 검색" [pressed] [ref=e45] [cursor=pointer]:
                - generic [ref=e46]: 우편번호 검색
            - textbox [ref=e50]:
              - /placeholder: 주소를 입력해 주세요,
            - textbox [ref=e54]:
              - /placeholder: 상세주소
          - generic [ref=e58]:
            - heading "유튜브 링크" [level=2] [ref=e59]
            - textbox [ref=e62]:
              - /placeholder: 링크를 입력해 주세요.
          - generic [ref=e66]:
            - heading "사진 첨부" [level=2] [ref=e67]
            - generic [ref=e68]:
              - img "사진업로드" [ref=e71] [cursor=pointer]
              - img "사진업로드" [ref=e74] [cursor=pointer]
              - img "사진업로드" [ref=e77] [cursor=pointer]
          - generic [ref=e80]:
            - button "취소" [pressed] [ref=e81] [cursor=pointer]:
              - generic [ref=e82]: 취소
            - button "등록하기" [pressed] [ref=e83] [cursor=pointer]:
              - generic [ref=e84]: 등록하기
  - alert [ref=e85]
  - generic [ref=e86]:
    - dialog "등록 완료":
      - generic [ref=e87]:
        - button "Close" [ref=e88] [cursor=pointer]:
          - generic "Close" [ref=e89]:
            - img "close" [ref=e90]:
              - img [ref=e91]
        - generic [ref=e94]: 등록 완료
        - paragraph [ref=e96]: 게시물이 성공적으로 등록되었습니다.
        - generic [ref=e97]:
          - button "Cancel" [ref=e98] [cursor=pointer]:
            - generic [ref=e99]: Cancel
          - button "확인" [ref=e100] [cursor=pointer]:
            - generic [ref=e101]: 확인
```